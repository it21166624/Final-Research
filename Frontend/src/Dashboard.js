...
const SyncIndicator = styled.div`
  position: absolute;
  top: 70px; /* push it below the header */
  right: 20px;
  display: flex;
  align-items: center;
...
const Dashboard = () => {
  ...
  const [stressPredict, setStressPredict] = useState(null);
  ...
  const fetchSensorData = async () => {
    try {
      ...
      let data = JSON.stringify({
        ...
        FeedIntakePerHen: sensorData?.feed_Quantity ?? 0,
        WaterIntakePerHen: 232.04,
        AirQuality: 1,
        Lighting: 10.28,
        CageDensity: 4.48,
        Vocalization: 68.24,
        BodyTemperature: sensorData?.bodyTemp,
        Heartbeat: sensorData?.heartRate,
      });

      let config = {
        method: "post",
        maxBodyLength: Infinity,
        url: "http://localhost:5000/predict_stress",
        headers: {
      ...
      axios
        ...
        .then((response) => {
          ...
          setStressPredict(response.data?.predicted_stress);
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  const handleDropdownChange = (event) => {
    ...
    if (selected === "Animal Stress") {
      navigate("/stress");
    } else {
      // Handle other options or stay on Dashboard
      // You could also add navigation to other pages if you want later
    }
  };

  // const GetStressStatus = async () => {
  //   let data = JSON.stringify({
  //     Temperature: sensorData?.temperature,
  //     Humidity: sensorData?.humidity,
  //     FeedIntakePerHen: sensorData?.feed_Quantity ?? 0,
  ...
  const handleSubmit = async () => {
    let data = JSON.stringify({
      ...
      Health_Status: stressPredict?.predicted_stress === 0 ?  "Healthy" : "Sick",
      ...
      <DashboardContainer style={{ paddingTop: "80px" }}>
        ...
        <StatsCard>
          ...
          <StatItem>
            ...
            <h3>Light Level</h3>
            <p>{sensorData?.light_Hours} Hrs</p>
            {/* <Badge type="normal">Normal</Badge> */}
          </StatItem>
